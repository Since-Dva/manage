...
export default new Vuex.Store({
  state: {
    allTabs: {
      jurisdiction:{tabs:[],activeIdx:0},
      account:{tabs:[],activeIdx:0},
      repair:{tabs:[],activeIdx:0},
      inquiry:{tabs:[],activeIdx:0},
      insurance:{tabs:[],activeIdx:0},
      contentM:{tabs:[],activeIdx:0},
      autoParts:{tabs:[],activeIdx:0},
    }
  },
  mutations: {
    addTabMut(state, data) {
      let {type,path,title}=data
      if (state.allTabs[type].tabs.length) {
        ...
        if(bool){
          state.allTabs[type].tabs.push({ path, title })
